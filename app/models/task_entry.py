...
class TaskEntry(Base):
    ...
    client_id = Column(UUID(as_uuid=True), ForeignKey("clients.id"), nullable=True)  # Now optional - clients tracked per sub-entry
...
class TaskSubEntry(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    task_entry_id = Column(UUID(as_uuid=True), ForeignKey("task_entries.id", ondelete="CASCADE"), nullable=False)
    client_id = Column(UUID(as_uuid=True), ForeignKey("clients.id"), nullable=False)  # NEW: Client per sub-task
    task_master_id = Column(UUID(as_uuid=True), ForeignKey("task_masters.id"), nullable=True)
